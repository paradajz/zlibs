...
bool EmuEeprom::format()
{
    ...
    if (_use_factory_page && (page_status(Page::Factory) == PageStatus::Valid))
    {
        for (uint32_t offset = 0; offset < CONFIG_ZLIBS_UTILS_EMUEEPROM_PAGE_SIZE; offset += ENTRY_SIZE_BYTES)
        {
            auto data = _hwa.read_32(Page::Factory, offset);
            ...
            if (!_hwa.write_32(Page::Page1, offset, data.value()))
            {
                return false;
            }
        }
...
uint32_t EmuEeprom::max_address() const
{
