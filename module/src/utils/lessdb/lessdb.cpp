...
bool LessDb::set_layout(std::span<const Block> layout, uint32_t start_address)
{
    ...
    uint32_t total_usage = layout.back().BLOCK_OFFSET + layout.back().byte_size();
...
uint32_t LessDb::current_database_size() const
{
    const LockGuard LOCK(_mutex);
    ...
    return _layout.back().BLOCK_OFFSET + _layout.back().byte_size();
}
