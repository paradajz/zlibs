    ...
    , _kwork_reset_movement([this]()
                            {
...

                                {
                                    ...
                                    // use kwork to avoid calling functions outside of this module in interrupt
                                    ...
                                    {
                                        // state unknown - make sure it's off
                            ...
                            zlibs::common::internal::Workqueue::handle())
{}
...
bool Motor::calculate_next_pulse()
{
    ...
    if (_movement.maxRunTime)
    {
        if (run_time() >= _movement.maxRunTime)
        {
            _run_state = RunState::Stopped;
        }
    }
