...
namespace zlibs::utils::sysex_conf
{
    ...
    /**
     * @brief Decoded fields extracted from one incoming SysEx message.
     */
    ...
    constexpr inline uint8_t SYSEX_START_BYTE = 0xF0;

    /** @brief SysEx frame end byte (`0xF7`). */
    constexpr inline uint8_t SYSEX_END_BYTE = 0xF7;
    ...
    /** @brief Generic 8-bit mask (`0xFF`). */
    constexpr inline uint8_t BYTE_MASK = 0xFF;

    /** @brief MIDI data-byte mask (`0x7F`). */
    constexpr inline uint8_t MIDI_DATA_MASK = 0x7F;

    /** @brief Index of the high bit in one byte. */
    constexpr inline uint8_t HIGH_BIT_INDEX = 7;
    ...
    class Merge14Bit
    {
        public:
        /**
         * @brief Constructs a 14-bit value from two encoded 7-bit bytes.
         *
         * @param high Encoded high transport byte.
         * @param low Encoded low transport byte.
         */
        constexpr Merge14Bit(uint8_t high, uint8_t low)
        {
            if (high & 0x01)
            {
                low |= (1 << HIGH_BIT_INDEX);
            }
            else
            {
                low &= ~(1 << HIGH_BIT_INDEX);
    ...
    class Split14Bit
    {
        public:
        /**
         * @brief Splits one 14-bit value into two encoded 7-bit bytes.
         *
        ...
        constexpr explicit Split14Bit(uint16_t value)
        {
            ...
            new_high         = (new_high << 1) & MIDI_DATA_MASK;

            if ((new_low >> HIGH_BIT_INDEX) & 0x01)
            {
                new_high |= 0x01;
            }
            else
            ...
            new_low &= MIDI_DATA_MASK;
            _high = new_high;
            _low  = new_low;
        }

        /**
         * @brief Returns the high 7-bit transport byte.
         */
        ...
        /**
         * @brief Returns the low 7-bit transport byte.
         */
