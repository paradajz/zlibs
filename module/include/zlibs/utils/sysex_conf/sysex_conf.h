...
namespace zlibs::utils::sysex_conf
{
    ...
    class SysExConf
    {
        ...
        /**
         * @brief Processes one incoming UMP SysEx7 packet.
         *
         * Multi-packet SysEx7 messages are reassembled internally before the
         * protocol decoder is invoked.
         *
         * @param packet SysEx7/Data64 UMP packet.
         */
        void handle_packet(const midi_ump& packet);
         ...
         *
         * Configured manufacturer-ID bytes are emitted automatically and the
         * response is sent as SysEx7/Data64 UMP packets.
         *
        ...
        bool                           _ump_message_active               = false;
        std::span<const Block>         _layout                           = {};
        std::span<const CustomRequest> _custom_requests                  = {};
        DecodedRequest                 _decoded_request                  = {};
        ...
        /**
         * @brief Processes one reassembled SysExConf request.
         *
         * This is the internal dispatcher after UMP SysEx7 packet reassembly.
         * It validates framing/manufacturer/status, dispatches special or
         * standard requests, and emits any response packets.
         *
         * @param message Encoded SysEx frame.
         */
        void handle_request(std::span<const uint8_t> message);

        /**
         * @brief Decodes fields for one standard SysExConf request.
         *
         * This only extracts and validates protocol fields such as wish,
         * amount, block, section, index, and new value. It does not check
         * framing/manufacturer bytes and does not send responses.
         *
         * @param message Incoming message bytes.
         *
         * @return `true` on successful decode/validation, otherwise `false`.
         */
        bool decode_standard_request(std::span<const uint8_t> message);

        /**
         * @brief Resets decoded-request scratch fields to default state.
         */
        void reset_decoded_request();

        /**
         * @brief Resets UMP SysEx7 reassembly state.
         */
        void reset_ump_message();
