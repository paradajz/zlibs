...
namespace
{
    ...
    class EmuEepromTest : public ::testing::Test
    {
        ...
        HwaTest   _hwa;
        EmuEeprom _emu_eeprom = EmuEeprom(_hwa, false);
    };
}    // namespace
...
TEST_F(EmuEepromTest, FormatCopiesFactoryPageWhenEnabled)
{
    ...
    EmuEeprom emu_eeprom_with_factory(_hwa, true);

    ASSERT_TRUE(emu_eeprom_with_factory.format());
    ASSERT_EQ(PageStatus::Valid, emu_eeprom_with_factory.page_status(Page::Page1));
    ASSERT_EQ(PageStatus::Formatted, emu_eeprom_with_factory.page_status(Page::Page2));
    ASSERT_EQ(ReadStatus::Ok, emu_eeprom_with_factory.read(0, page1_value));
    ASSERT_EQ(ReadStatus::Ok, emu_eeprom_with_factory.read(1, page2_value));
...
TEST_F(EmuEepromTest, FormatFailsWhenFactoryCopyReadFails)
{
    set_page_status(Page::Factory, PageStatus::Valid);
    write_raw_flash_entry(Page::Factory, 0, 0, 0x1234);
    ...
    EmuEeprom emu_eeprom_with_factory(_hwa, true);
    ASSERT_FALSE(emu_eeprom_with_factory.format());
}
