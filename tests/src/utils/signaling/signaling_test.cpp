...
TEST_F(SignalingTest, DerivedSignal)
{
    ...
    // first run: first subscriber will always receive data
    // second subscriber has a filter on c field - data will be pushed onto a derived signal only if c field value is Val2
    Data data = {};
    data.a    = 1243;
    data.b    = -2;
    ...
    // second subscriber should now receive the data and pass on the filtered data onto a derived signal
    data.c = TestEnum::Val2;
    ...
    // the received derived signal count should remain the same because the pushed values are the same
    publish_and_wait(data);
...
TEST_F(SignalingTest, AccumulateMaintainsIndependentStatePerSubscription)
{
    ...
    auto accumulated_signal = signaling::observe<Data>().accumulate(
        0,
