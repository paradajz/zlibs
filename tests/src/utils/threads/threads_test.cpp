...
namespace
{
    ...
    using CancelUserThread   = threads::UserThread<misc::StringLiteral{ "cancel-thread" }, K_PRIO_PREEMPT(0), 1024>;
...
TEST_F(ThreadsTest, KworkDelayableCancelWaitsForRunningCallbackToComplete)
{
    ...
    k_sem allow_completion   = {};
    k_sem callback_completed = {};
    k_sem cancel_entered     = {};
    k_sem cancel_returned    = {};
    k_sem_init(&callback_started, 0, 1);
    k_sem_init(&allow_completion, 0, 1);
    k_sem_init(&callback_completed, 0, 1);
    k_sem_init(&cancel_entered, 0, 1);
    k_sem_init(&cancel_returned, 0, 1);

    auto* workqueue = TestWorkqueue::handle();
    ASSERT_NE(nullptr, workqueue);

    misc::KworkDelayable work([&]()
                              {
                                  k_sem_give(&callback_started);
                                  ASSERT_EQ(0, k_sem_take(&allow_completion, K_FOREVER));
                                  k_sem_give(&callback_completed);
                              },
                              [workqueue]()
                              {
                                  return workqueue;
    ...
    CancelUserThread cancel_thread([&]()
                                   {
                                       k_sem_give(&cancel_entered);
                                       work.cancel();
                                       k_sem_give(&cancel_returned);
                                   });

    ASSERT_TRUE(work.reschedule(0));
    ASSERT_EQ(0, k_sem_take(&callback_started, K_MSEC(WAIT_TIME_MS)));
    ASSERT_TRUE(cancel_thread.run());
    ASSERT_EQ(0, k_sem_take(&cancel_entered, K_MSEC(WAIT_TIME_MS)));

    // Desired behavior: cancellation should not return until the active handler exits.
    ASSERT_NE(0, k_sem_take(&cancel_returned, K_MSEC(10)));

    k_sem_give(&allow_completion);
    ASSERT_EQ(0, k_sem_take(&callback_completed, K_MSEC(WAIT_TIME_MS)));
    ASSERT_EQ(0, k_sem_take(&cancel_returned, K_MSEC(WAIT_TIME_MS)));
}
